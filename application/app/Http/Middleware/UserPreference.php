...
class UserPreference extends Controller
{
	...
	public function MyProfile(Request $request){
        ...
        return view('UserPreference.profile', compact('activeID', 'user','year','month','day'));

	}
	public function updateProfile(Request $request){
		$user = Tbluser::where('email', '=', session()->get('userEmail'))->first();
		$user->title = $request->input('title');
		$user->country = $request->input('location');
        $user->straddress1 = $request->input('street-1');
        $user->straddress2 = $request->input('street-2');
        $user->gender = $request->input('title');
...
public function generatePDF($booking_id){
    ...
    $hotel_state =$record->hotel_state;
    $image  =      $hotels["image"];
    $rating = $hotelPolicy->hotel->starRating;
   
    $data=array();
    $data['bookingDate']=$record->bookingDate;
    $data['chekIn'] =$record->chekIn;
    ...
    $data['hotel_state']=$hotel_state;
    $data['image'] =$image;
    $data['hote_fee']=$record->hote_fee;
    $pdf = PDF::loadView('UserPreference.MakePDF', compact('data'));
    $pdf->setPaper('A4', 'landscape');
//    \Mail::send('UserPreference.MakePDF', ['bookingdetail'=>$data], function($message) use($pdf)
//    {
//        $message->from('bcs.12.43@gmail.com', 'Your Name');
//
//        $message->to('techleadz.cfm@gmail.com')->subject('Invoice');
//
//        $message->attachData($pdf->output(), "invoice.pdf");
//    });
//    $path = storage_path('mail/')."booking.pdf";
//    $pdf->save($path);
	  ...
	  if ($validator->fails()) {
         ...
         if($flag){
             ...
             if ($checked){
                 ...
                 return redirect('user/password')->with('message', 'Password is not changed, Fill all fields correctly ');
             }
			}
			else {
				return redirect('user/password')->with('message', 'Current Password is not matched');
			}

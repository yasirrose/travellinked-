...

{
    ...
    public function ViewHotel(Request $request,$city,$id){
        ...
        $request->session()->set('currenthotelStatus',1);

        $images = Helper::getHdImages($id, $this->APiUser);


        $url = "http://api.bonotel.com/index.cfm/user/".$this->APiUser."/action/hotel/hotelCode/".$id;
        ...
        $currHotelDest = DB::table('hotel')
            ...
            ->where('hotel.hotelCode',$id)

            ->first();


        if($currHotelDest==null){
            $currHotelDest = Helper::getNewHotelToDatabase($id, $this->APiUser);
...

        {
            ...
            $topBarData['text'] = $hotelPolicy->hotel->city.'';

            $citySlug = str_replace(' ','-',$hotelPolicy->hotel->city);

            $cityCode = $hotelPolicy->hotel->cityCode;

            $topBarData['link'] = '/cities/'.$citySlug.'/'.$hotelPolicy->hotel->hotelCode.'/'.$cityCode;

        }

        $retHotel['name'] = $hotelPolicy->hotel->name.'';
        $retHotel['city'] = $hotelPolicy->hotel->city.'';
        $retHotel['stateProvince'] = $hotelPolicy->hotel->state.'';
        $retHotel['country'] = $hotelPolicy->hotel->country.'';

        $retHotel['image'] = $hotelPolicy->hotel->images->image[0].'';
        $retHotel['starRating'] = $hotelPolicy->hotel->starRating.'';
        $retHotel['allImages'] = $images;

        $retHotel['hotelCode'] = $hotelPolicy->hotel->hotelCode.'';
        ...
        foreach($roomBeds->hotel->room as $room){
            foreach($room->bed as $bed){
                $item = array();
                $item['roomType'] = $room->roomName.'';
                $item['bedType'] = $bed->bedName.'';
                $item['stdAdults'] = $bed->adults.'';
                $item['stdChild'] = $bed->children.'';
                $item['roomDescription'] = '';
                $rooms[] = $item;
            }
        }
        $retHotel['rooms'] = $rooms;
        $viewHotelfullUrl =  \Request::fullUrl();
        $request->session()->set('viewHotelroomDetail',4);
        $request->session()->set('viewHotelfullUrl',$viewHotelfullUrl);
        $request->session()->set('viewHotelrooomHotelCode',$retHotel['hotelCode']);

        return view('frontend.viewHotel')->with('retHotel', $retHotel)->with('section', $section)->with('topBarData', $topBarData)->with('hotelPolicy', $hotelPolicy)->with('viewHotelfullUrl',$viewHotelfullUrl);
    }
    public function goToHotelLandingPage(Request $request,$city,$id)

    {
...

        {
...

            {
                ...
                $checkInObj->modify('+10 day');

                $checkOutObj->modify('+12 day');
            ...
            foreach($Harr as $hotel){
                ...
                if(isset($currMinDeal[0]['rateInformation']['totalRate'])){
                    if(Helper::getOptimalRate($currMinDeal) > $maxPrice){
                        ...
                        if($minPrice > Helper::getOptimalRate($currMinDeal)){
                            $minPrice = Helper::getOptimalRate($currMinDeal);
                        }
                    }
                }else{
    ...
    public function goToSearchPageCitites(Request $request,$city,$citycode, $newcityCode)
...
//
...

                {
                    ...
                    if($minPrice > intval(str_replace(',','',$prices['rateInformation']['totalRate']))){
                        $minPrice = intval(str_replace(',','',$prices['rateInformation']['totalRate']));
                    }
                }

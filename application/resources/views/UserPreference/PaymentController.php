...

{
...

	{
...

		{
...

			{
				...
				$taxAmount = floatVal(($totalRate*$actualTax)/100);

				$discountAmount = floatVal(($actualDis*$totalRate)/100);

				$withAdminTax = floatVal($totalRate+$taxAmount-$discountAmount);

				$addTax = $actualTax/100;

				$addDis = $actualDis/100;

				$pertTax = $addTax-$addDis;

				$percentTax = intval($pertTax*100)."%";
...

			{
				...
				$withAdminTax = floatVal($totalRate+$actualTax-$actualDis);

				$perTax = $actualTax - $actualDis;
...

		{
			...
			$grandTotal = round($withAdminTax + $tax + $hotel_fee,2);
...

		{
			...
			$grandTotal = round($totalRate + $tax + $hotel_fee,2);

		}

		$amountToPay = round($totalRate + $tax,2);
		...
		return view('frontend.payment',compact("hotel", "resortFee",  "nights","checkin","checkout","adults","childs","cityName","dispRooms","address","image","hotel_fee","tax","percentTax","actualTax","totalRate","totalrooms","grandTotal","policyFrom","policyTo","amendmentType", "hotelPolicy"));
...

	{
		...
		/*======== session set for search result page if back ====================*/
...

				{
...

					{
...

						{
							...
							$policyFrom  .=   $room["roomBookingPolicy"]["policyFrom"].",";

							$policyTo 	.=   $room["roomBookingPolicy"]["policyTo"].",";

							$amendmentType .= $room["roomBookingPolicy"]["amendmentType"].',';
		...
		return view('frontend.payment',compact("hotel","nights","checkin","checkout","adults","childs","cityName","dispRooms","address","image","hotel_fee","tax","percentTax","actualTax","totalRate","totalrooms","grandTotal","policyFrom","policyTo","amendmentType"));
...

	{
		...
		include(app_path().'/Api/brain-tree/global.php');
...

		{
...

			{
				...
				"roomCodes"=>$rcode,"is_visitor"=>1,"chekIn"=>$traveldate,"checkOut"=>$traveldateend,

				"bookingDate"=>$book_date,"is_pending"=>1,"hotel_name"=>$hotels["name"],"no_rooms"=>$nRooms,"no_nights"=>$nights,

				"room_types"=>$rType,"adults"=>$rAdults,"children"=>$rChild,"tax"=>$rTax,"total_amount"=>$amount,
...

				{
					...
					$paymentID = DB::table("room_booking")->insert(["taxes"=>$rTax,"adminTaxes"=>$adminTax,"bookingID"=>$result->transaction->id,

					"request_id"=>$reqID,"bookingType"=>$result->transaction->type,

					"currency"=>$result->transaction->currencyIsoCode, "amount"=>$amount,"amountWithTax"=>$amount,

					"merchantAccountId"=>$result->transaction->merchantAccountId,"bookingDate"=>$date,
...

			{
				...
				$reqID = DB::table("tblpending_requests")->insertGetId(["adminTax"=>$adminTax,"user_id"=>$userId
...

		{
			...
			$html1 = "<ul style='color:white;padding:10px;border-radius:10px;background:red;'><li>Payment failed.</li><li>Error Message : $result->message</li></ul>";
...

	{
		...
		return view("frontend.confirm",compact("roomsDetail","room_booking","travelDetail","bookingDetail","billingDetail","hotels","rooms","nights","hotelPolicy","userDetail","policyFrom","policyTo","amendmentType"));

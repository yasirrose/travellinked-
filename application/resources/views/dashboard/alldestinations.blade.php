...
<div class="content-wrapper">
    <div class="content-heading"> <em class="fa  fa-map-marker" aria-hidden="true"></em> <span class="admin-breadcrumb"><a href="#">Destinations </a>  </span>   </div> @include('flash.flash')
    <div class="row">
        <div class="col-lg-12">
            <!-- START panel-->
...
<script type="text/javascript">
    ...
    function getValue(xyz){
        ...
        $('#dest').DataTable().ajax.url('{{url('admin/destinations')}}?value='+filter_record_var).load();
    }
    ...
    $(document).ready(function () {
        var table =  $("#dest").dataTable({
            "bProcessing" : true,
            "sAjaxSource" : "{{url('admin/destinations')}}?value="+filter_record_var,
...
//        $('.' + currForm).submit();
     ...
     $.ajax({
         type : "POST",
         url : "{{url('admin/update-destination-status')}}",
    ...
    function submitform(counter){
        ...
        if( $('#file'+counter).val() != ""){
            ...
            $.ajax({
                url:"{{url('admin/update-destination-image')}}", // Url to which the request is send

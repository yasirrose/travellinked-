...
var rooms =  $("#rooms_count").val();
  for(var i = 1; i<= rooms; i++){
	var val = $("#adult-"+i).val();
   if(val == 0){
		 $("#adult-"+i).addClass("error");
 ...
 submitHandler: function(form) {
  ...
  for(var i = 1; i<= rooms; i++){
   ...
   /*
    $("#searchb-tn").click(function(e){
	 e.preventDefault();
	 
 	var hotel = $(".search_hotel").val();
	var checkin = $("#datepicker_in").val();
	var checkout = $("#datepicker_out").val();
	...
	var val = $("#adult-"+i).val();
	 ...
	 if(val == 0){
		...
		//return false;
		  ...
		  $("#adult-"+i).removeClass("error");
		  $(".search-form").submit();
		 }
   }
	 /*if(hotel == "" || checkin == "" || checkout == "" || nights == ""){
		 
		  $(".search_hotel").addClass("error");
		  $("#datepicker_in").addClass("error");
		  $("#datepicker_out").addClass("error");
...
$("#datepicker_out").change(function(){
 ...
 /*==== Datepicker date auto fill in chekout field========*/
 $("#datepicker_in").change(function(){
	var  checkin = $(this).val(); 
	var checkin= $("#datepicker_in").datepicker('getDate');
	checkin.setDate(checkin.getDate() + 1);
	var Newdate = checkin.getDate() + '/' + (checkin.getMonth() + 1) + '/' +  checkin.getFullYear();
		$("#datepicker_out").val(Newdate);
 });
/*==== End Datepicker date auto fill in chekout field========*/
 ...
 $(".star-filter").change(function(){
	...
	$(".vacation").addClass("inactive");
	 ...
	 $.ajax({
		...
		$(".overlay").hide();
		var c = '<input type="hidden" value="1" id="'+star+'">';
		$(".filter-data").html(data+c);
		}		
	}
...
$( "#slider-range" ).slider({
		...
		$.ajax({
			type : "GET",
			url : site_url + "/FilterByslider",
			data : {slider : $("#amount").val()},
			dataType : 'json',
			cache : false,
		success : function(data) {
